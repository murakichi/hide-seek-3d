...
/**
 * 箱の高さ。**どちらも目線（`EYE_HEIGHT` = 1.4）より高く、視線を止める。**
 *
 * 視線は目線から相手の頭（同じ高さ）へ引くので、1.4 以下の箱は遮蔽として
 * まったく働かない。小さい箱は長らく 1.3 で、**箱の 6 割が見た目どおりに
 * 隠してくれない**状態だった。準備フェーズで 32 秒かけて積んだ砦が素通しになる。
 *
 * `CLIMB_REACH`（1.64）以下に収めてあるので、「小さい箱には地面から直接乗れる」
 * 性質は変わらない。踏み台としても遮蔽としても使える。
 *
 * 2026-08-10 に一度 1.5 を試して取り下げている（3 構成そろって 2〜4 ポイント低下）。
 * **当時と前提が違う。** あの頃は逃げ側が 24.7/35.3/38.3% で帯の下寄り、
 * 鬼は迎撃（PR #18）も目撃情報の分散（PR #23）も持っていなかった。
 * その後 鬼が大幅に強くなり、逃げ側は 27/10/1% まで落ちている。
 * 実測は `docs/balance-log.md` の 2026-08-11 の節。
 */
export const BOX_HEIGHT_SMALL = 1.5;
