...
/**
 * ダッシュ消費は両陣営とも同じ。**差は速度だけ**（逃 9.4 / 鬼 8.8）で付ける。
 *
 * 以前はここを 52（逃げる側の 1.5 倍）にして「鬼は息が続かない」を作っていたが、
 * 2026-08-11 に測ったところ **鬼が追いつけないことが確定していた**。
 * 回復は共通 19/秒なので、持続できるダッシュの割合は逃げ側 19/34 = 56%、
 * 鬼 19/52 = 37%。使い切ったときの平均速度は逃げ側 12.3 m/s に対して鬼 10.6 m/s で、
 * 鬼はダッシュ率 83%・スタミナ 23 まで削っても構造的に届かなかった。
 *
 * 消費を揃えても「速い方が勝つ」にはならない。鬼の方が遅いので、
 * 逃げる側が全力で走れば依然として振り切れる（同条件で 11.5 対 12.3）。
 * 逃げる側が今それをしていないだけ（issue #10）。
 * `docs/balance-log.md` 2026-08-11 の節に 52 / 42 / 34 の実測がある。
 */
export const SEEKER_DASH_COST = 34;
