 ...
 *
 * **視覚ではないので、遮蔽も煙も貫く**（`src/core/vision.ts` の `canSee`）。
 * 以前は視覚と同じ条件で遮蔽に切られていたが、それだと
 * 「箱の陰に隠れる」と「鬼の接近に気づく」が両立せず、隠れるほど無防備になる。
 * `BOX_HEIGHT_SMALL` を上げる変更が 2 回とも逆効果だったのはこれが原因ではないか、
 * という仮説から変えた。実測は `docs/balance-log.md` の 2026-08-11 の節。
 */
