...
/**
 * 鬼 1 人あたりの視界距離。**盤面あたりの探索能力を人数によらず一定にする。**
 *
 * 視界の面積は距離の 2 乗なので、鬼が n 人なら合計の探索能力は n 倍になる。
 * アリーナの広さは人数によらず 44x44 のままなので、人数が増えるほど
 * 盤面が「見尽くされる」速さだけが上がっていた。
 *
 * 実測でもそこが効いていた。逃げる側が視線を切ってから再発見されるまでの時間は
 * 1v1 3.9 秒 / 2v2 2.6 秒 / 3v3 2.5 秒（issue #43）。
 * **見失う回数も 6.7 → 4.5 → 3.4 回に減る。**「常に見えている」のではなく、
 * 「切っても 2.5 秒で別の鬼に拾われる」ので逃げ切りに使える時間が無い。
 *
 * `VIEW_DIST / √n` にすると合計面積が人数によらず一定になる。
 * 1v1 19.0（変更なし）/ 2v2 13.4 / 3v3 11.0。
 *
 * **逃げる側には掛けない。** これは鬼の探索能力に対する補正であって、
